...
- **INV-2** 目标进程不在白名单 → 必须经**本地审批入白**（ISS-0012，三态：本次允许=会话 / 永久加入=系统写盘 / 拒绝），无人类批准则拒绝（`APPROVAL_DENIED`）；自保护进程（deskpilot.exe 自身）永不可入白，硬拒 `NOT_WHITELISTED`；面向 AI 的拒绝文本只陈述未授权，不授任何加入方法。**补充（ISS-0072）：撤回即人类否决，永久有效**——已被人类从白名单移出的进程（盘面墓碑 `max_level: null`）收到任何操作请求一律硬拒 `REVOKED_BY_HUMAN`，零弹窗、零取图、零抢焦点，不得再走三态入白审批；恢复授权的唯一通道是**人类**在白名单管理窗口中主动加回。撤回态须与"素未谋面"在强制层可区分（`Policy.revoked` → `WhitelistAdmin.is_revoked`）
