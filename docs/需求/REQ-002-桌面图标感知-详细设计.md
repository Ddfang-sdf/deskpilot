...
通用性(三 provider 各自可注入);source 按名归并(ISS-0054:序对齐假设 A-2 已作废——枚举序分叉实机实证;索引仅用于同视图同源的 LVIR 图形矩形)。
...
| 按名归并 | UIA 显示名 ↔ PIDL 文件名(规范化:小写去空白、剥 .lnk/.url)归并进 source;序对齐假设 A-2 已经 ISS-0054 实证作废 |
...
| DesktopIconAssembler | 三路计数一致性校验、按名归并 source、region 过滤、错误归约 | 三 provider、Executor |
...
| 成员 | 语义 |
|------|------|
| `graphic_rect(index)` → rect | 跨进程消息:远程内存写入 LVIR_ICON 标记的矩形结构→发 LVM_GETITEMRECT→读回→**经 ClientToScreen 客户区原点平移为虚拟桌面坐标**(ISS-0075:LVM_GETITEMRECT 返回 client 坐标,原点平移是显式收口;原点读不出=显式报错,fail-closed 不回退未转换值) |
...
| 成员 | 语义 |
|------|------|
| `assemble(region=None)` → [IconEntry] | 三路计数一致性校验→按名归并 source(重名歧义/无匹配来源 fail-closed)→region 相交过滤→返回 |
...
| 归并证据 | 三路 | 计数一致 + 归并键命中 | 计数不一致/重名歧义/无匹配来源均 fail-closed 报错停手 |
...
mcp_server._call(L0 直调)
  └─ tools._run_sensing ─ Executor.list_desktop_icons(region)
       └─ DesktopIconAssembler.assemble
            ...
            ├─ 计数一致性校验(三路 count 一致;不一致→停手报错)
            ├─ 按名归并 source(重名歧义/无匹配来源→停手报错)合成 IconEntry×N
...
sequenceDiagram
    ...
    E->>E: 计数一致性校验→按名归并 source→region 过滤
...
### 9.4 归并校验(mermaid,ISS-0054 回填)

```mermaid
flowchart TD
    A[三路各自产出] --> B{三路 count 一致?}
    B -- 否 --> C[报错:计数不一致<br>含三路 count 直出]
    B -- 是 --> D{逐项按名归并键命中来源?}
    D -- 重名歧义 --> E[报错:归并失败-图标重名歧义<br>附改名指引]
    D -- 无匹配来源 --> F[报错:归并失败-无匹配来源<br>附上报指引]
    D -- 命中/虚拟项 --> G[合成 IconEntry<br>虚拟项 source=null]
```

说明:source 归并按归并键(UIA 显示名 ↔ PIDL 文件名;规范化=小写去
空白、剥 .lnk/.url 后缀,ISS-0054)。序对齐假设 A-2 作废:IEnumIDList
枚举序与 UIA 可视序在自定义排列/桌面编辑后分叉(实机实证);LVIR 图形
矩形与 UIA 同视图同源,索引序天然一致,graphic_rect 仍按索引配。
计数一致性校验保留(fail-closed);首项同名检查已删(被名归并取代)。
...
| 三路计数不一致 | INTERNAL_ERROR | 含三路 count 直出,停手 |
| 归并失败(图标重名歧义/无匹配来源) | INTERNAL_ERROR | 含图标名直出+指引,停手 |
...
| 接缝 | 用途 |
|------|------|
| 三 provider 各自可注入 | 单元以替身断言归并/过滤/计数校验/报错归约(替身计数直出) |
| ContainerLocator 可注入 | 替身返回假句柄/抛异常,覆盖 WorkerW 与 fail-closed 分支 |
| region 相交纯函数 | 相交判定单元用例(直出) |
| 归并键规范化与 fail-closed 分支 | 重名歧义/无匹配来源两分支(直出) |
