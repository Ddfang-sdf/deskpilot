...
| 成员 | 语义 |
|------|------|
| `to_virtual(rect, 原点)` → 虚拟屏 rect | 逐分量平移:`l+ox, t+oy, r+ox, b+oy`。**不做缩放**(截图为区域原样抓取,落盘图 scale 恒 1.0,与 `screenshot` 落盘/`scale_x/scale_y` 契约一致;screenshot MCP 内联预览图长边 >2000 时等比降采样、scale=f<1,见 ISS-0089 A3——检测器读落盘全分辨率原图,不受影响) |
...
### 5.2 条目形态(SOM-02,统一键集;ISS-0066 ②修订:UIA 条目增 `som_id`)

| 键 | UIA 条目 | 检测区域条目 |
|----|---------|-------------|
| `id` | 整数,同一空间内唯一 | 同左 |
| `som_id` | 整数,与 `id` 同值(ISS-0066 ②双写:输出↔`click_element` 入参命名对齐;`id` 标废弃日程,下个大版本删) | **无此键**(detect 编号不可寻址,只取 rect 用 click) |
| `source` | `"uia"` | `"detect"` |
| `name` | 控件名 | **恒 `null`** |
...
说明:**不做缩放**(落盘图 scale 恒 1.0,与 `screenshot` 落盘契约一致;MCP 内联预览降采样语义见 ISS-0089 A3)。收口点是本
