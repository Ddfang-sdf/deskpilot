...
| 状态 | **建单(sdfang 2026-09-18 已裁定整改原则),方案待评审** |
...
- 关联暴露面请架构师裁定:①CLI `--reset` / HTTP `/estop/reset`(localhost
  接口,AI 可 curl)是否保留为人类通道或同步收口;②复位热键
  Ctrl+Shift+F11(物理输入通道;冻结期 key 工具已拒,但 shell 级 SendInput
  可模拟)去留。
