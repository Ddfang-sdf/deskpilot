...
| 状态 | **已关闭**(2026-09-23 验收通过:三态点击实盘——拒绝=APPROVAL DENIED 审计在案;本次允许=session L2+真实拉起;永久加入=落盘 policy.local.yml(ISS-0030 双文件)+热生效;授权自决) |
...
| v0.12 | 2026-09-23 | **验收通过关单**(sdfang 离场留言授权自决,记录在案)。证据:三态点击实盘——拒绝=APPROVAL DENIED 审计在案;本次允许=session L2+真实拉起;永久加入=落盘 policy.local.yml(ISS-0030 双文件)+热生效(原「待 sdfang 完成三态点击与撤回手工验收」已完成) |
...
### appnames（F 补丁，新增模块 `deskpilot/appnames.py`）

| 入口 | 签名 | 语义 |
|------|------|------|
| `app_display_name` | `(process: str, window_title: str = "") -> str` | 进程→用户可读显示名（纯函数）：① window_title 非空原样返回；② 按进程名解析 exe 全路径（PATH/App Paths）读版本信息 FileDescription；③ 全失败返回进程名本身。ctypes 零新依赖；单次 <100ms |
