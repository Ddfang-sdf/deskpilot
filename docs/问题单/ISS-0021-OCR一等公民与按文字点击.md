...
| 状态 | **实盘验收完成**(2026-09-04:click_text 西柚二连击全中+OCR_TEXT_NOT_FOUND 摘要+元数据实盘；457 用例全绿） |
...
| v0.3 | 2026-09-03 | 评审通过,P1 红→P2 核对→P3 绿落地:resolve_click 纯函数+executor._click_text(与 click 同防误射链)+tools/models/mcp 注册+screenshot 元数据;**设计修正记录:index 默认 None=未指定**(默认 0 会使 OCR_AMBIGUOUS 分支永不可达);ocr() 增区域 dict 直通;TC-CT-08 魔法串改逐次唯一(会话恢复带回旧串实证);8 用例全绿,全量 457 |
| v0.4 | 2026-09-04 | **测试放宽登记(双闸门补)**：验收期 TC-CT-08 因 type_text 读回滞后双贴导致多命中,曾将断言放宽为 index=0——当时未入册,违反放宽双闸门;双贴缺陷已由 ISS-0035【修改引入】修复(读回轮询确认),本单 ct08 已恢复唯一性断言并兼任双贴回归探测器。轨迹留此备查 |
