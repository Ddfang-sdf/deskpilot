...
| 状态 | **已关闭(2026-09-23 验收:三钉全绿+落盘文本 diff 零差异;离场授权自决)** |
...
| v0.3 | 2026-09-22 | 裁决批准(按§5 推荐项):①常量形态=模块级 EV_*(不采 class 类属性);②41 个值中英文混排保持原样,值英文化涉历史数据迁移策略另立单;③S3 测试断言常量化强制,文本钉由 TC-60-03 单点承担;④S2 按 12 文件逐个提交;⑤读审计 helper 4 份复制另立 cleancode 单,本单不动。离场授权自决,记录在案;状态→方案已批准,待开发 |
| v0.4 | 2026-09-23 | **S0~S4 全部执行完毕(17 提交)**。①S0(65de73c):三钉先红(TC-60-01 ast 结构守卫实测红 57 处/TC-60-02/03 空壳红)。②S1(ced46b0):audit_events.py 46 常量逐字转录,TC-60-02/03 转绿。**口径登记:§0 表 41 种+表落档后 ISS-0094(心跳写失败/恢复)/ISS-0095(管理窗拉起/名称缓存暖机/白名单数据装配)新增 5 种=46 种**;41 常量清单经 TC-60-02(46 数/非空/互异)+TC-60-03(46 键值逐字钉板)双钉确认。③S2(8ae15a8/d1408bc/c77b0fd/34cfc2b/129b171/7fcdd76/6921dbb/3f8830d/04b3a0a/051db6d/e9f2511/f2bc850):12 源文件逐文件常量化(三形态全覆盖:secure_desktop 条件表达式两分支/main 双默认参+1 传参/whitelist_admin _event 包装调用点);**S2-13(4f25ce0)补漏登记**:§0 十二文件表后 ISS-0095 落地三事件调用点遗漏补常量化(httpd.py 为第 13 源文件),secure_desktop IfExp 常量化后仍触守卫条件形态→调用前定名;三钉全绿。④**落盘文本 diff 实证**:git worktree 对照(fe21791 动工前 vs S2 完成)同场景 EstopMonitor 触发/复位/noop——急停触发/急停复位/复位请求-未冻结三高频事件 JSONL **逐字节零差异**(DIFF_IDENTICAL)。⑤S3(7dc7b8d):19 测试文件 57 处断言/驱动常量化(§0 表 15+表后新增 4 文件差额登记:heartbeat/managerobs/dualfile_fix_iss32/guardfix_iss34);57 处=grep 实测逐一对账。⑥S4:audit.record_event docstring 指向词汇表。⑦每步双跑(EXIT 门禁),全量终态 **935 passed 0 failed 29 skipped**(基线 932+3 新钉)。⑧过程知悉:test_whitelist_iss12::test_post_clear_session 曾现单发 flaky(单跑即绿,排序敏感,非本单引入) |
