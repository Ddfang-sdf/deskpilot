...
| 状态 | **已关闭**(2026-09-23 验收通过:心跳写健壮性+tmp pid 化落地 912 绿;B-2 静默拦截在 ISS-0064 S9 打包实证中坐实机制(新构建写 LOCALAPPDATA 不落盘、写 C:	mp 正常、Defender 日志无事件=第三方安全软件拦截,观测面消歧已入档);E-1/E-2 实证项转 ISS-0108 跟踪面;授权自决) |
...
## 5. 裁决(2026-09-22,sdfang 离场留言授权自决,记录在案)

- **E-5:§4.5 立项,批准开发**,范围=第 1 条(心跳写健壮性)+第 2 条(tmp 按 pid 区分)+第 3 条(观测面消歧文档);第 4 条(端口加固)随 ISS-0108 另行验证后定,不在本次开发范围;
- **E-4 → 另立 ISS-0107**(audit_dir 锚定);**E-3 → 另立 ISS-0108**(跨会话双绑验证);E-1/E-2 实证项随 ISS-0095 O 面落地后择机(杀软日志读取不影响本单开发);
- 状态转「方案已评审批准,待开发(P1)」。

### 观测面消歧(批准范围第 3 条,验收/运维判读口径)

- **heartbeat 内容=当前属主**:文件里出现旧 pid/旧 version ≠「不写」——旧属主
  (存活或僵死持锁)在场时,新实例按设计抢锁失败退出,心跳面如实保留旧属主内容;
- **判活一律以 `ts` 新鲜度为准**(`is_daemon_alive` 语义:<30s 为活),不以
  pid/version 字段推断「哪个版本在跑」;
- **onefile 父子对**:冻结形态 `deskpilot.exe` 恒为两个进程(onefile 引导器父进程+
  真 daemon 子进程,§4.2 实证)——进程观测以 **9420 持端者**为真 daemon,父进程
  不是僵尸也不是第二属主;
- **estop-state.json 无周期写设计**:冻结/解冻边沿、启动清理、对账修复才写
  (§4.3),「不冻不写」是预期,旧时间戳不等于失效。

## 6. 变更记录

| 版本 | 日期 | 内容 |
|------|------|------|
| v0.2 | 2026-09-22 | §4 诊断结论落档(测绘/父子对实证/estop-state 澄清/根因表 B-1~B-6/§4.5 方案/§4.6 裁决清单);req 面随 ISS-0093 废止 |
| v0.3 | 2026-09-22 | **裁决(离场授权自决)**:§4.5 立项批准(范围=心跳健壮性+tmp pid 化+观测消歧;端口加固随 ISS-0108);E-3→ISS-0108、E-4→ISS-0107 立单;状态转待开发 |
| v0.4 | 2026-09-22 | **P1→P3 全循环完成回填**。①P1:tests/test_heartbeat_iss94.py(TC-94-01~05)落码,红分布精确——TC-94-01/02 现状无重试首次 OSError 即上抛零审计、TC-94-03 tmp 统一名无 pid;TC-94-04/05 绿(行为保持面正确落点)。②P2 自检:逐用例核对入口(beat_once/RoleSupervisor.start/main --daemon)、断言出处(桩 sleep 序列/审计桩记录/盘上 glob+JSON/返回值,全直出)、五要素齐全,不一致 0 项。③P3 实现(ownership.py):`beat_once` 重写——OSError 三段退避重试(0.05/0.15/0.45s,每轮预清同名旧 tmp),终败返回 None 不上抛(start() 直连调用方,心跳不阻断主功能);节流审计「心跳写失败」(连续失败只记首败,恢复另记「心跳写恢复」,审计自身失败不上抛,`_record_failure/_record_recovery/_audit_event`);tmp 按 pid 区分(`daemon-heartbeat.{pid}.tmp`,构造期捕获 self._pid,payload 同源一致);RoleSupervisor.start/tick 两处构造接线 audit(main.py daemon/stdio 装配 audit 自然流入);「抢锁失败停心跳」语义未动(TC-94-04 钉绿)。④测试数字:P1 3 failed 2 passed;P3 后受影响面(test_heartbeat_iss94+test_ownership_iss84)**18 passed**;全量默认层 **912 passed 0 failed**(基线 907+5 新用例)。⑤观测面消歧(批准第 3 条):本单据 §5 下新增「观测面消歧」节(heartbeat 内容=当前属主,旧 pid≠不写;判活以 ts 新鲜度为准;onefile 父子对以 9420 持端者为准;estop-state 不冻不写是预期)+docs/INSTALL.md 故障排查表同步一行。⑥范围确认:端口加固未做(ISS-0108 另案);B-2 拦截源打包实证留 E-1/E-2 择机;freeze_notify/_loop 其他面未动 |
...
| v0.3 | 2026-09-23 | **验收通过关单**(sdfang 离场留言授权自决,记录在案)。证据:心跳写健壮性+tmp pid 化落地 912 绿;B-2 静默拦截在 ISS-0064 S9 打包实证中坐实机制(新构建写 LOCALAPPDATA 不落盘、写 C:	mp 正常、Defender 日志无事件=第三方安全软件拦截,观测面消歧已入档);E-1/E-2 实证项转 ISS-0108 跟踪面 |
