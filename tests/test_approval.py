...
class TestRequestAndConsume:
    def test_approve_issues_token_and_consumed_once(self, approvals, approver):
        """TC-N-APR-01：批准→签发；按指纹查证通过一次；再次查证拒绝（一次性）。"""
        approver.decision = True
        fp = compute_fingerprint("key", {"key": "alt+f4", "token": "t1"})
        token = approvals.request_approval("按键 alt+f4 作用于测试窗口", fp)
    ...
    def test_deny_or_timeout_returns_none(self, approvals, approver):
        """TC-E-ST-04：不批准/无响应 → 无令牌签发。"""
        approver.decision = False
        fp = compute_fingerprint("key", {"key": "delete"})
        assert approvals.request_approval("desc", fp) is None
        assert approvals.count() == 0

    def test_expired_token_rejected(self, approvals, approver, clock):
        """TC-S-TOK-02：超过 approval_ttl 后查证拒绝。"""
        approver.decision = True
        fp = compute_fingerprint("key", {"key": "delete"})
        assert approvals.request_approval("desc", fp) is not None
    ...
    def test_failed_verify_does_not_consume(self, approvals, approver):
        """TC-S-TOK-04：指纹不匹配查证失败不消耗令牌，原样重试仍放行。"""
        approver.decision = True
        fp = compute_fingerprint("key", {"key": "delete"})
        ...
        assert approvals.verify_and_consume(fp) is True          # 令牌仍在，可消费
        assert approvals.count() == 0
...
class TestM1DenyAll:
    def test_deny_all_channel_always_denies(self, clock):
        ...
        assert mgr.request_approval("desc", fp) is None
