...
class TestScope03Coverage:
    ...
    def test_fullscreen_vs_screen_difference_visible(self, ex):
        ...
        assert (r_full["virtual_rect"][2] - r_full["virtual_rect"][0]) * \
               (r_full["virtual_rect"][3] - r_full["virtual_rect"][1]) \
               == BOUNDING_AREA
        assert BOUNDING_AREA > 1920 * 1080  # 外接矩形 > 单屏(虚空存在)
...
class TestScope05Regression:
    ...
    def test_fullscreen_unchanged(self, ex):
        ...
        assert r["virtual_rect"] == [0, -1, 3840, 1080]
