...
锁屏或 UAC 弹窗（安全桌面）期间，AI 的**一切**操作——含截屏等只读感知——都会被结构化拒绝（`SECURE_DESKTOP`），你回到桌面后自动恢复。**人类不可视 = AI 不可动**。
...
- 🛑 **急停有感知** —— 热键/甩角熔断一切写操作，冻结主动弹窗告知；**解冻是人类专属动作**（设计如此）
- 🖥️ **多显示器原生** —— 弹窗跟随你正在操作的屏；截图与元素树用虚拟桌面坐标（含负值）；按屏截图屏号确定（0=主屏，其余从左到右）
...
AI 客户端 ──MCP(stdio)──▶ deskpilot ──四道闸硬校验──▶ Windows 桌面
                              │
                              ├─ 危险操作 → 本地审批窗（你点头才执行）
                              ├─ 急停熔断 → 冻结通知弹窗（解冻仅人类可操作）
                              └─ 全程审计 → 截图 + JSONL 留痕
```

29 个 MCP 工具（截图 / OCR / 按文字点击 / 元素树 / 点击 / 输入 / 窗口管理……）。安全模型、四道闸细节、协议设计的完整文档在 [docs/](docs/DESIGN.md)。
...
- 锁屏 / UAC 弹窗期间，AI 的**全部**调用（含只读）返回 `SECURE_DESKTOP`，回到桌面自动恢复——你看不到的时候，AI 也动不了
